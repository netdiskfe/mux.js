 ...
 * @file:   flv.js
 * @author: zhengjiu
 * @email:  zhengjiu@baidu.com
 * @date:   2016-09-18 16:13:31
 * @last modified by:   zhengjiu
 * @last modified time: 2017-02-30 22:27:11
 */
...
FlvParseStream = function() {
  ...
  var parseAudioData = function(data, start, end) {
    ...
    if (soundFormat === 10) {
      ...
      result.data = data.subarray(start + 2, end);
    }
  ...
  var parseVideoData = function(data, start, end) {
    ...
    var codecID = (videoHeader & 0x0f);     // & 1111
    ...
    if (codecID === 7) {
      ...
      if (packetType === 1) {
        ...
        if (compositionTime & 0x00800000) {   // why we should do this?
            compositionTime |= 0xff000000;
        }
        result.cts = compositionTime * 90;
      }
      result.data = data.subarray(start + 5, end);
    }
  ...
  var parseTag = function(data, start) {
    var result = {};
    // audio(08 / 0x08)  video(09 / 0x09)  scriptdata(18 / 0x12)
...
ElementaryStream = function() {
  ...
  var parseVideoConfig = function(data) {
    ...
    sidx += 2;
    result.sequenceParameterSetNALUnits = data.subarray(sidx, sidx + sequenceParameterSetLength);
    ...
    sidx += 2;
    result.pictureParameterSetNALUnits = data.subarray(sidx, sidx + pictureParameterSetLength);
    return result;
  };
      ...
      if (data.packetType === 0) {
        ...
        while (i < len) {
          ...
          this.trigger('data', {
            ...
            pts: data.pts,
            dts: data.dts,
            data: array.subarray(i, i + unitLen),
            config: videoConfig
          });
          i += unitLen;
        }
...
AdtsStream = function() {
  ...
  this.push = function(packet) {
    ...
    if (packet.type !== 'audio') {
      // ignore non-audio data, AdtsStream just for audio data parse
      return;
    }
...
H264Stream = function() {
  ...
  // Find the EmulationPreventionBytes and remove them.
  // To prevent there have 0x00 0x00 0x01 in the inner of NALUnit,
  // in the begin of the H.264 encode, H.264 will add 0x03 behind 0x00 0x00.
  // So, if found bytes sequence like 0x00 0x00 0x03, we must remove this
  // EmulationPreventionBytes(0x03) and restore to the original state.
  ...
  this.push = function(packet) {
    ...
    if (packet.type !== 'video') {
      // ignore non-video data, H264Stream just for video data parse
      return;
    }
    ...
    if (event.nalUnitType === 'slice_layer_without_partitioning_rbsp_idr') {
      ...
      // skip first byte, and remove EmulationPreventionBytes
      var escapedRBSP = discardEmulationPreventionBytes(sequenceParameterSetNALUnits.subarray(1));

      // use Golomb Decoder to parse sequenceParameterNALUnit,
      // then we can get the config about the video, just like the
      // video's width, video's height and other.
