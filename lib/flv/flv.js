 ...
 * @last modified time: 2016-09-20 15:09:00
 */
...
FlvParseStream = function() {
  ...
  var parseFileHeader = function(data) {
    var result = {};

    if (data.length < MIN_FILE_HEADER_BYTE_COUNT) {
    ...
    result.headerLength = new DataView(data.slice(5, 9).buffer).getUint32(0);

    return result;
  };

  var parseAudioData = function(data, tagDataLength) {
    ...
    if (soundFormat === 10) {
      result.codec = 'aac';
      result.packetType = data[1];
      result.data = data.slice(2, tagDataLength);
    }

    return result;
  };

  var parseVideoData = function(data, tagDataLength) {
    ...
    if (codecID === 7) {
      ...
      result.data = data.slice(5, tagDataLength);
    }

    return result;
  };

  var parseTag = function(data) {
    var result = {};
    var type = data[0];
    var tagDataLength = (data[1] << 16 | data[2] << 8 | data[3]);

    switch (type) {
      case TAG_TYPE.AUDIO:
        result = parseAudioData(data.slice(TAG_HEADER_BYTE_COUNT), tagDataLength);
        break;
      case TAG_TYPE.VIDEO:
        result = parseVideoData(data.slice(TAG_HEADER_BYTE_COUNT), tagDataLength);
        break;
    ...
    result.dts = (data[7] << 24) | (data[4] << 16) | (data[5] << 8) | (data[6]) * 90;
    if (result.cts) {
    ...
    while (start < len) {
      switch (state) {
        case PARSE_STATE.FILE_HEADER:
          result = parseFileHeader(data.slice(start));
        ...
        case PARSE_STATE.TAG:
          result = parseTag(data.slice(start));
          if (result.codec) {
  ...
  this.push = function(data) {
    ...
    if (data.codec === 'avc') {
      if (data.packetType === 0) {
        result = parseVideoConfig(data.data);
        lengthSizeMinusOne = result.lengthSizeMinusOne;
        this.trigger('data', {
          type: data.type,
          trackId: data.trackId,
          pts: data.pts,
          dts: data.dts,
          data: result.sequenceParameterSetNALUnits
        });
        this.trigger('data', {
          type: data.type,
          trackId: data.trackId,
          pts: data.pts,
          dts: data.dts,
          data: result.pictureParameterSetNALUnits
        });
      } else {
        var array = data.data;
        var i = 0;
